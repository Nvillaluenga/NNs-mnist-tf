...
#     weights = {
#     #5 x 5 convolution, 1 input, 32 outputs
#     'w_conv1': tf.Variable(tf.random_normal([5,5,1,32])),
#     #5x5 conv, 32 inputs,64 output
#     'w_conv2': tf.Variable(tf.random_normal([5,5,32,64])),
#     # 7*7*64 in, 1024 outputs
...
#
#         correct = tf.equal(tff.argmax(prediction, 1),tf.argmax(y, 1))
#
#         accuracy = tf.reduce_mean(tf.cast(correct, 'float'))
#         print('Accuracy:',accuracy.eval({x: data.test.images, y: data.test.labels}))
#
# our_model = convolutional_neural_network(x, keep_rate)
# train_neural_network(our_model,mnist)
